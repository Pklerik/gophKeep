...
// Config contain attrs for DB configuration.
type Config struct {
	Dialect   string
	User      string
	Password  string
...
func (dbc *Config) UnmarshalText(text []byte) error {
...
func (dbc *Config) SetDefault() error {
	if _, ok := dbc.Options["search_path"]; !ok {
		dbc.Options["search_path"] = "gophkeep"
	}
...
func (dbc *Config) GetConnString() string {
...
func (dbc *Config) GetUser() string {
	return dbc.User
}

// GetOptions return Options.
func (dbc *Config) GetOptions() Options {
	return dbc.Options
}
